...
impl SlashCommand for WorkflowCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        if trimmed.is_empty() {
            return CommandResult::Message(self.usage().to_string());
        }

        let mut parts = trimmed.splitn(2, char::is_whitespace);
        let first = parts.next().unwrap_or_default();
        let rest = parts.next().unwrap_or_default().trim_start();
        if is_manage_op(first) && !first.eq_ignore_ascii_case("runs") {
            return CommandResult::Action(Action::WorkflowManage {
                op: first.to_ascii_lowercase(),
