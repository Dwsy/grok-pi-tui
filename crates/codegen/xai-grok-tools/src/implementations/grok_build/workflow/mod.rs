...
pub enum WorkflowSource {
    Name {
        #[schemars(
            description = "Name of a registered workflow (built-in, or discovered from the project `$GROK_PROJECT_DIR/workflows/` (default `.grok` / grok-pi `.grok-pi`) or user `$GROK_HOME/workflows/`)."
        )]
        name: String,
    },
    Script {
