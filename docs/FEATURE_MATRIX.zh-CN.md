...
| 浏览器配置台（`/pi-config web`、`/pi-models web`） | Pi 扩展（Node） | 注入的 `pi-grok-web-config` 扩展把资源管理、provider/model 管理和 F2 设置以网页形式提供：随机回环端口 + 每次启动一次性 token + `Host` 校验 + 结构校验 + tmp/rename 原子写入。F2 行与原生一一映射：`grok-pi.json` 烘焙清单 + 其余 `[ui]` 标量键；TOML 重写只动 `[ui]`、保留 `config.toml` 其余字节。扩展独占读写（`models.json`、`settings.json`、`config.toml [ui]`、`ctx.reload()`、`pi.setModel()`）；界面中/英随浏览器语言自动切换。Rust 仅负责转发参数与注入 bundle。 |
